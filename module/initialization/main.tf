...
resource "null_resource" "initialize_root_domain" {
  provisioner "local-exec" {
    command = <<EOT
      helm install root-domain --repository-config ${path.module}/../../data/helm/config/repositories.yaml --repository-cache ${path.module}/../../data/helm/cache/repository \
      -f ${path.module}/../../data/helm/values/spaceone-initializer/root.yaml \
    ...
    command = <<EOT
      ...
      do
          ...
          if [[ $domain_type =~ root-domain ]] && [[ $status =~ Completed ]]; then
              ...
              helm install user-domain --repository-config ${path.module}/../../data/helm/config/repositories.yaml --repository-cache ${path.module}/../../data/helm/cache/repository \
              -f ${path.module}/../../data/helm/values/spaceone-initializer/user.yaml \
              spaceone/spaceone-initializer
