...
resource "local_file" "generate_frontend" {
  content  =  templatefile("${path.module}/tmpl/frontend.tpl",
    {
      console-api-domain                 = "${data.terraform_remote_state.certificate.outputs.console_api_domain}"
      console-api-domain-certificate-arn = "${data.terraform_remote_state.certificate.outputs.console_api_certificate_arn}"
      console-domain                     = "${data.terraform_remote_state.certificate.outputs.console_domain}"
      console-domain-certificate-arn     = "${data.terraform_remote_state.certificate.outputs.console_certificate_arn}"
      spaceone-version                   = "${var.spaceone-version}"
    })
  filename = "${path.module}/yaml/frontend.yaml"
}
...
resource "local_file" "generate_value" {
  ...
  filename = "${path.module}/yaml/values.yaml"
}
