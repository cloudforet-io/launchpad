...
function clean_up_configure() {
    ...
    if [ -f "$BASE_WORKING_DIR/data/tfstates/$component.tfstate" ]; then
        rm -rf $BASE_WORKING_DIR/data/tfstates/$component.tfstate*
    fi
}
...
function destroy() {
    ...
    for ((i=$len -1; i>=0; i--))
    do
        if [ -f $BASE_WORKING_DIR/data/tfstates/${components[$i]}.tfstate ]; then
...
function main() {
    ...
    if [ $cmd == "install" ]; then
        ...
        helm upgrade spaceone $( for value_file in `ls $BASE_WORKING_DIR/data/helm/values/spaceone`; do echo $value_file | sed -e "s/^/-f $BASE_WORKING_DIR_sed\/outputs\/helm\/spaceone\//g"; done ) spaceone/spaceone
    else
